...
def compute_pitch_pytorch(wav, sample_rate):
    ...
    pitch_feature = torchaudio.functional.compute_kaldi_pitch(wav, sample_rate,frame_length=64,frame_shift=20)
...
def compute_pitch_pyworld(wav, sample_rate, hop_length, pitch_fmax=640.0):
    ...
    f0 = pw.stonemask(wav, f0, t, sample_rate)
    return f0
...
class DurationPitchPredictor(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.to_duration_pred = DurationPitchPredictorTrunk(
            dim = dim_hidden,
            depth = depth,
            kernel_size = kernel_size,
            dim_context = dim_encoded_prompts,
    ...
    def forward(
        ...
        '''
        duration_pred=self.to_duration_pred(x)
        pitch_pred=self.to_pitch_pred(x)
...
class Model(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if self.condition_on_prompt:
            self.cond_to_model_dim = nn.Conv1d(196, 192, 1)
