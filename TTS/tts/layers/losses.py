...
class VitsGeneratorLoss(nn.Module):
    ...
    def asr_loss(self, tokens, tokens_len, ctc, z_len):
        #assert torch.all(z_len > tokens_len).item(), "有的语句长度大于语音长度"

        #target=torch.torch.sum(tokens_len)

        loss = self.ctc_loss(ctc.transpose(0, 1).log_softmax(2), tokens, z_len, tokens_len)
        return loss
    ...
    def forward(
        ...
        # compute losses
        loss_lf0 = torch.nn.functional.mse_loss(pred_lf0, lf0)
        loss_feat = (
        ...
        loss_mel = torch.nn.functional.l1_loss(mel_slice, mel_slice_hat) * self.mel_loss_alpha*2
        #loss_duration = torch.sum(loss_duration.float()) * self.dur_loss_alpha

        loss_asr=self.asr_loss(tokens,tokens_len,soft,z_len)*self.asr_loss_alpha


        mask=z_mask.unsqueeze(1)


        reconstruction_loss=torch.nn.functional.mse_loss(x,y)*4.5

        vq_loss=vq_loss

        loss =  loss_feat + loss_mel + loss_gen+vq_loss +reconstruction_loss+loss_lf0+ loss_asr
        ...
        return_dict["loss_lf0"] = loss_lf0
        return_dict["loss_mel"] = loss_mel
        return_dict["loss_asr"] = loss_asr
        return_dict["vq_loss"]=vq_loss
...
class FineturnLoss(nn.Module):
    def __init__(self,c:Coqpit,blank):
        super().__init__()
    ...
    def asr_loss(self,tokens,tokens_len,ctc,z_len):
        loss = self.ctc_loss(ctc.transpose(0,1).log_softmax(2), tokens, z_len,  tokens_len)
        return loss
    def forward(self,loss_duration,loss_AR,bert_result,indices,mask):#,ctc,indices):
        return_dict = {}


        loss_duration = torch.sum(loss_duration.float())
        #print(logs_p.required_grad)
        loss_bert = self.criterion((bert_result.log_softmax(dim=2)).transpose(1, 2) * mask,
                                   (indices * mask.squeeze(1)).long())




        loss=loss_duration+loss_bert+loss_AR

        return_dict["loss_AR"] = loss_AR
        return_dict["loss_bert"] = loss_bert
