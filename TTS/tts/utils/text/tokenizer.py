...
from TTS.tts.utils.text.phonemizers import DEF_LANG_TO_PHONEMIZER, get_phonemizer_by_name, JA_JP_Phonemizer
...
class TTSTokenizer:
    ...
    def text_to_ids(self, text: str, language: str) -> List[int]:  # pylint: disable=unused-argument
        ...
        if language == "ja":
            text = e.phonemize(text)
        elif language == "en":
            text = backoff.trans_list(text)
        elif language == "ch":
            text = backoff.trans_list(text)
        else:
            raise "语言粗哟了"
