...
def aishell1(root_path,meta_files=None,wavs_path=None,ignored_speakers=None):
    items = []
    text_file_path=os.path.join(root_path,"transcript","aishell_transcript_v0.8.txt")
    with open(text_file_path, encoding="utf-8") as f:
        while True:
            ...
            filename=tmp[0:16]
            speaker_id=tmp[6:11]
            wave_file=os.path.join(root_path,"wav",speaker_id,filename+".wav")
            if os.path.exists(wave_file):
                items.append(
                    {"text": text, "audio_file": wave_file, "speaker_name": "AISHELL1_" + speaker_id, "root_path": root_path}
                )
            else:
                print(f" [!] wav files don't exist - {wave_file}")
    return items

def jvs(root_path,meta_files=None,wavs_path=None,ignored_speakers=None):
    items = []
    meta_files = glob(f"{root_path}/**/**/*.txt", recursive=True)
    for meta_file in meta_files:
        speaker_id,staly, txt_file = os.path.relpath(meta_file, root_path).split(os.sep)
        #file_id = txt_file.split(".")[0]
        if staly[0]=="f":
            continue
        with open(meta_file, "r", encoding="utf-8") as f:
            while True:
                ...
                if text[-1]=="\n":
                    text=text[:-1]
                wave_file=os.path.join(root_path,speaker_id,staly,"wav24kHz16bit",tmp.split(":")[0]+".wav")
                if os.path.exists(wave_file):
                    #resample_file((wave_file, 16000))
                    items.append(
                        {"text": text, "audio_file": wave_file, "speaker_name": speaker_id,
                         "root_path": root_path}
                    )
                else:
                    print(f" [!] wav files don't exist - {wave_file}")
...
def kespeech1(root_path,meta_files=None,wavs_path=None,ignored_speakers=None,k=1):
    items = []
    text_path=os.path.join(root_path,"Metadata","phase1.text")
    path_path=os.path.join(root_path,"Metadata","phase1.wav.scp")
    lang_path=os.path.join(root_path,"Metadata","phase1.utt2subdialect")
    f1=open(path_path,"r",encoding="utf-8")
    f2=open(text_path,"r",encoding="utf-8")
    f3=open(lang_path,"r",encoding="utf-8")
    while True:
        path=f1.readline()
        if not path:
            break
        path_tmp=path[:-1].split(" ")


        text=f2.readline()
        if not text:
            break
        text_tmp=text[:-1].split(" ")

        lang=f3.readline()
        if not lang:
            break
        lang_tmp=lang[:-1].split(" ")

        wave_file=os.path.join(root_path,path_tmp[1])
        assert path_tmp[0] == text_tmp[0] and path_tmp[0]==lang_tmp[0],(path_tmp[0],text_tmp[0],lang_tmp[0])

        if os.path.exists(wave_file):
            items.append(
                {"text": text_tmp[1], "audio_file": wave_file, "speaker_name": "KESPEECH_" + text_tmp[0][:7],"dialect":lang_tmp[1],
                 "root_path": root_path}
            )
    f1.close()
    f2.close()
