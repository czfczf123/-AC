...
from TTS.tts.utils.synthesis import synthesis, transfer_voice, trim_silence, embedding_to_torch
    ...
    def tts(
        ...
        if self.tts_languages_file or (
            ...
            if len(self.tts_model.language_manager.name_to_id) == 1:
                ...
                print(self.tts_model.language_manager.name_to_id)
                try:
        ...
        if speaker_wav is not None:
            ...
            speaker_embedding = encoder_manager.compute_embedding_from_clip(speaker_wav)
        ...
        return wavs
    def vc(self,src,tar):
        x, sr = torchaudio.load(src)
        if sr!= 16000:
            resample_file((src, 16000))
        reader = WhisperFeatureReader(root=None, ckpt="medium", layer=24, device="cpu")
        tmp = reader.get_feats(src)
        #tmp=torch.load("/media/D/czf/KeSpeech/whisper_results/KESPEECH_1010481/1010481_4ee86ae9.whisper.pt",map_location='cpu')



        tmp=tmp.transpose(0,1).unsqueeze(0)
        print("tmp:", tmp.shape)
        if self.use_cuda:
            tmp=tmp.cuda()
        x, sr = torchaudio.load(tar)
        if sr != 16000:
            resample_file((tar, 16000))
        print("tmp:", tmp.dtype)
        print(self.use_cuda)
        ...
        speaker_embedding=encoder_manager.compute_embedding_from_clip(tar)

        # = self.tts_model.speaker_manager.compute_embedding_from_clip(tar)
        d_vector = embedding_to_torch(speaker_embedding, cuda=self.use_cuda)

        wav, sr = librosa.load(src, sr=16000)
        ...
        y = wav_to_spec(x, 1024, 320, 1024, center=False)


        y_lengths=torch.tensor([y.shape[2]]).to(tmp.device)
        print(y.shape)


        c_lengths = torch.tensor([tmp.shape[2]]).to(tmp.device)
        aux_input={
            "d_vectors": d_vector,
        }


        output=self.tts_model.inference_vc(tmp,c_lengths,y.to(tmp.device),y_lengths.to(tmp.device),aux_input)
        #output = self.tts_model.inference_voice_conversion(tmp, c_lengths, f0.unsqueeze(0), energy.unsqueeze(0), x, aux_input)
