...
class VectorQuantize(nn.Module):
    ...
    def set_codebook(self,cb):
        self.embed.data.copy_(cb.transpose(0,1))
...
class ResidualVQ(nn.Module):
    ...
    def lookup(self, indices):
        quantized_out = F.embedding(indices, self.layers[0].codebook)  # Num x T x C
        return torch.sum(quantized_out, dim=0, keepdim=True)
    def set_codebook(self,cb):
        return self.layers[0].set_codebook(cb)
    def get_codebook(self):
        return self.layers[0].codebook
