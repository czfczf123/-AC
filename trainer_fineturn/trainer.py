...
class Trainer:
    def __init__(  # pylint: disable=dangerous-default-value
        ...
        parse_command_line_args: bool = False,
        ...
        Args:
            ...
            parse_command_line_args (bool):
                ...
                to false if you parse the arguments yourself. Defaults to False.
    ...
    def _model_train_step(
        ...
        return model.fineturn_train_step(*input_args)
    ...
    def _optimize(
        ...
        if self.use_amp_scaler:
            ...
            if step_optimizer:
                self.callbacks.before_gradient_clipping(self)
                if grad_clip[0] > 0:
                    grad_norm = torch.nn.utils.clip_grad_norm_(self.master_params(optimizer), grad_clip[0])
                optimizer.step()
    ...
    def train_step(self, batch: Dict, batch_n_steps: int, step: int, loader_start_time: float) -> Tuple[Dict, Dict]:
        ...
        if self.args.rank == 0:
            ...
            if self.total_steps_done % self.config.save_step == 0 and self.total_steps_done != 0:
                ...
                elif hasattr(self.model, "fineturn_train_log"):
                    self.model.fineturn_train_log(
                        batch,
                        outputs,
                        self.dashboard_logger,
                        self.training_assets,
    ...
    def _model_eval_step(
        ...
        return model.fineturn_eval_step(*input_args)
    ...
    def eval_epoch(self) -> None:
        ...
        if self.args.rank == 0:
            ...
            elif hasattr(self.model, "fineturn_eval_log"):
                self.model.fineturn_eval_log(
                    batch,
                    outputs,
                    self.dashboard_logger,
                    self.training_assets,
    ...
    def get_optimizer(model: nn.Module, config: Coqpit) -> Union[torch.optim.Optimizer, List]:
        ...
        if hasattr(model, "get_optimizer"):
            try:
                optimizer = model.fineturn_get_optimizer()
            except NotImplementedError:
                optimizer = None
    ...
    def get_lr(model: nn.Module, config: Coqpit) -> Union[float, List[float]]:
        ...
        """
        lr = None
        if hasattr(model, "fineturn_get_lr"):
            try:
                lr = model.fineturn_get_lr()
            except NotImplementedError:
                lr = None
        if lr is None:
            lr = config.lr
    ...
    def get_scheduler(
        ...
        """
        scheduler = None
        if hasattr(model, "fineturn_get_scheduler"):
            try:
                scheduler = model.fineturn_get_scheduler(optimizer)
            except NotImplementedError:
                scheduler = None
    ...
    def get_criterion(model: nn.Module) -> nn.Module:
        ...
        criterion = model.fineturn_get_criterion()
        return criterion
    ####################
    # HELPER FUNCTIONS
    ####################
    ...
    def _pick_target_avg_loss(self, keep_avg_target: KeepAverage) -> Dict:
        ...
        if "target_loss" in self.config and self.config.target_loss:
            ...
            return keep_avg_target["avg_loss"]
