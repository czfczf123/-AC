...
def maximum_path_numpy(value, mask, max_neg_val=None):
    ...
    for j in range(t_y):
        ...
        #print("index_mask",index_mask.shape)#1 187
    ...
    return path
def minmum_path_yinsu_numpy(value, mask):
    """
    Monotonic alignment search algorithm
    Numpy-friendly version. It's about 4 times faster than torch version.
    value: [b, t_x, t_y]上三角矩阵
    mask: [b, t_x, t_y]
    """
    print("我写的名为我骄傲")
    ...
    aa=np.ones_like(value)
    d_mask=np.tril(aa)
    value=np.where(d_mask,max_pos_val,value)
    ...
    for i in range(2,t_x):
        value[:,0,i]=value[:,0,i-1]+value[:,0,i]

    print("fdas ",value)


    for i in range(1,t_y):
        for j in range(i,t_x):
            if i==j:
                direction[:,i,j]=1
                value[:,i,j]=value[:,i-1,j]
            else:
                max_mask=(value[:,i,j-1]+value[:,i,j])<value[:,i-1,j]
                value[:,i,j]=np.where(max_mask,value[:,i,j-1]+value[:,i,j],value[:,i-1,j])
                direction[:,i,j]=max_mask


        '''
        v0 = np.pad(v, [[0, 0], [1, 0]], mode="constant", constant_values=max_pos_val)[:, :-1]
        v1 = v
        v0=v0+value[:,i,:]
        max_mask = v1 >= v0
        v_max = np.where(max_mask, v0, v1)
        direction[:, :, i] = max_mask

        index_mask = x_range <= (t_y-i-1)
        # print("index_mask",index_mask.shape)#1 187
        v = np.where(index_mask, v_max, max_pos_val)
        '''
    print(value)
    print("direaction", direction)
    direction = np.where(mask, direction, 1)
...
def tanxin_yinsu_numpy(value, mask):
    """
    Monotonic alignment search algorithm
    Numpy-friendly version. It's about 4 times faster than torch version.
    value: [b, t_x, t_y]上三角矩阵
    ...
    index_range = np.arange(b,dtype=np.int32)
    x_index = np.zeros_like(index_range)
    for i in range(0,t_y-1):
        mask1=(value[index_range,x_index,i+1]<20)
        print(mask1)
        print(x_index)
        x_index=np.where(mask1,x_index,i)
        print(i)
        direction[index_range, x_index, i + 1] = 1
    direction = direction * mask.astype(np.float32)
    direction = torch.from_numpy(direction).to(device=device, dtype=dtype)
